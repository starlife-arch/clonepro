...
export default async function handler(req, context) {
  ...
  const userName = payload.name || payload.userName || memberId || userId;
  let db = null;
  try {
    db = getDb();
  } catch (error) {
    ...
    if (db) {
      try {
        ...
        const ticketRef = await db.collection('tickets').add({
          ...
          messages: [
            { from: 'user', name: userName, text: userMessage, time: new Date().toISOString() },
        ...
        if (userEmail) {
          try {
            await fetch(`${process.env.URL || process.env.SITE_URL}/.netlify/functions/send-email`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json', 'x-api-token': process.env.EMAIL_API_TOKEN || '' },
              body: JSON.stringify({
                type: 'ticket_created',
                to: userEmail,
                data: { name: userName || 'Member', ticketId, subject: 'Human support requested', message: userMessage, severity: 'medium' },
              }),
            });
          } catch (e) { console.error('[ai ticket email] failed', e); }
        }
