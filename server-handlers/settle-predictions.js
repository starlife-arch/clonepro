...
async function sendPush(userId, title, body, url, type) {
  try {
    await fetch(`${process.env.URL || process.env.SITE_URL || 'https://starlifeadvert.com'}/api/send-push`, {
      method: 'POST',
...
async function legacyHandler(req, context) {
  ...
  try {
    ...
    for (const bet of bets.filter(b => b.outcomeId !== winningOutcomeId)) {
      ...
      await sendPush(bet.userId, '😔 Result In', `${event.title} has ended. Your prediction was not correct this time`, `${process.env.URL || process.env.SITE_URL || 'https://starlifeadvert.com'}/#`, 'predictions_result');
    ...
    for (const bet of bets.filter(b => b.outcomeId === winningOutcomeId)) {
      ...
      await sendPush(bet.userId, '🏆 You Won!', `Your prediction on ${event.title} was correct — $${netPayout.toFixed(2)} added to Game Wallet`, `${process.env.URL || process.env.SITE_URL || 'https://starlifeadvert.com'}/#`, 'predictions_result');
