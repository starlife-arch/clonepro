...
async function legacyHandler(req, context) {
  ...
  for (const doc of snapshot.docs) {
    ...
    const legacyPrice = !alerts.monthlyPrice || Number(alerts.monthlyPrice) === 10;
    const noticeDate = alerts.expiresAt?.toDate ? alerts.expiresAt.toDate() : now;
    if (legacyPrice && !alerts.upgradeNoticeSent) {
      try { await sendMsgAlertEmail('msg_alerts_upgrade_notice', user.email, { name: user.name || user.username || 'Member', renewalDate: noticeDate.toLocaleDateString() }); } catch (e) { console.error('[msg alerts upgrade notice] failed', e); }
      await doc.ref.update({ 'msgEmailAlerts.upgradeNoticeSent': true, 'msgEmailAlerts.monthlyPrice': monthlyPrice });
    } else if (legacyPrice) {
      await doc.ref.update({ 'msgEmailAlerts.monthlyPrice': monthlyPrice });
    }
    ...
    if (Number(user.balance || 0) >= monthlyPrice) {
      await doc.ref.update({
        balance: admin.firestore.FieldValue.increment(-monthlyPrice),
        'msgEmailAlerts.active': true,
        'msgEmailAlerts.expiresAt': admin.firestore.Timestamp.fromDate(newExpiry),
        'msgEmailAlerts.lastRenewedAt': admin.firestore.FieldValue.serverTimestamp(),
        'msgEmailAlerts.lastFailedAt': null,
        'msgEmailAlerts.totalPaid': admin.firestore.FieldValue.increment(monthlyPrice),
        'msgEmailAlerts.monthlyPrice': monthlyPrice,
      });
      await db.collection('activity').add({ uid: doc.id, desc: 'Message Email Alerts renewal — $20.00', amt: -monthlyPrice, icon: '💬', type: 'subscription', createdAt: admin.firestore.FieldValue.serverTimestamp() });
      await db.collection('userNotifs').add({ uid: doc.id, msg: '✅ Message Email Alerts renewed for another 30 days. $20.00 deducted from your wallet.', read: false, createdAt: admin.firestore.FieldValue.serverTimestamp() });
      ...
      await db.collection('userNotifs').add({ uid: doc.id, msg: '⚠️ Your Message Email Alerts subscription has paused. Your wallet had insufficient funds. Top up $20 to resume.', read: false, createdAt: admin.firestore.FieldValue.serverTimestamp() });
