...
async function migrateRoles(report, db) {
  report.roles = await processBatches(db.collection('users').where('role', '==', 'admin'), 'roles', async (record) => {
    await query('UPDATE users SET role = $2, updated_at = NOW() WHERE id = $1', [record.id, 'admin']);
  });
}
